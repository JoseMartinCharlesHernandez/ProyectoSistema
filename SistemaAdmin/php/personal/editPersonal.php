...
$id_personal=$_POST['id_personal'];
     //consulta para traer el catalogo de los tipos de sangre
        $query=$pdo->prepare("SELECT * FROM personal WHERE id=:p");
        $query->bindParam(':p',$id_personal);
        $query->execute();
        $info_personal=$query->fetchAll(PDO::FETCH_ASSOC);
...
<div class="main-container-forms">
	<h1 style="margin-top: -2%;" class="h1Titulo"> <b>Editar personal</b></h1>
	<div class="cont_form">
		<?php  foreach ($info_personal as $datos) { 
			$info=$datos['id'];
			?>
	...
	<button onclick="enviarDatos('<?php echo $info; ?>')" class="btn btn-primary" type="button" id="btn_addSangre" data-toggle="modal" data-target="#modalAceptar">Aceptar</button>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
          <button onclick="location.href='../personal/personal.php'"; type="button" class="btn btn-primary">Aceptar</button>
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
      </div>
    </div>
  </div>
